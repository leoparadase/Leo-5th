...
public:
	...
	CryptoAPI()
	{
		if (!CryptAcquireContext(&m_hCP, NULL, MS_ENH_RSA_AES_PROV, PROV_RSA_AES, CRYPT_VERIFYCONTEXT))	// использование временных ключей, не сохран€ющихс€ в контейнере
			PrintError();
...
//		}
	...
	void EncryptData(ifstream& in, ofstream& out, DWORD dwSize, HCRYPTKEY hKey = NULL, bool bRSA = false)
		...
		// дл€ большинства алгоритмов можно использовать кратное значение,
		// но RSA требует точного соответстви€ длине блока в байтах,
		// причем 11 байт нужны дл€ об€зательного заполнител€ (padding)
	{
		if (!hKey)
			hKey = m_hSessionKey;
		DWORD dwBlockLen = 0;
